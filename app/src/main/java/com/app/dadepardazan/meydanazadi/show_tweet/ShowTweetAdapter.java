...
public abstract class ShowTweetAdapter extends RecyclerView.Adapter<ShowTweetAdapter.ViewHolder> {
    ArrayList<JSONObject> data_list;
    Context context;

    public ShowTweetAdapter(Context context, ArrayList<JSONObject> data_list) {
        this.context = context;
        this.data_list = data_list;
    }

    @Override
    public ShowTweetAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_show_tweet, parent, false);
        return new ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(ShowTweetAdapter.ViewHolder holder, final int position) {
        try {
